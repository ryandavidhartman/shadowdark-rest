...
- OpenAPI schemas are now concrete for content/generator endpoints (races, classes, items, etc.) instead of `GenericObject`, and include `/api/openapi.json` + `/api/docs` entries.
- OpenAPI components now mark required fields across schemas and include example payloads for common request/response types.
- OpenAPI endpoints now include per-endpoint request/response examples in addition to component-level examples.
...
- Character PDF helper script: `scripts/download-zero-level-character-pdfs.sh` downloads `/random-character.pdf?zeroLevel=true` repeatedly, extracts a likely full name from each rendered PDF via `pdftotext -layout`, and saves into `generated-characters/` as `FirstName LastName.pdf` with ` (n)` collision suffixes; if extraction fails it falls back to `random-character-N.pdf`.
