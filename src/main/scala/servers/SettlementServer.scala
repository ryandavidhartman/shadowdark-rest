...
final case class SettlementServer(
      ...
      case "Market" =>
        List(
          ...
          (
            5,
            5,
            PoiTemplate("Apothecary", "shop", shopQuality = Some("Standard"), shopTypeOverride = Some("Apothecary"))
          ),
  ...
  private def rollPoiTemplate(districtType: String): PoiTemplate = {
    ...
    table
      .collectFirst { case (min, max, poi) if roll >= min && roll <= max => poi }
  ...
  private def nudgePoiLocation(
    ...
    anchor match {
      ...
      case None =>
        ...
        while (
          attempts < 12 && (used
            .exists(p => distance(p, current) < minDistance) || !insideBoundary(current, boundaryPoly))
        ) {
          attempts += 1
          val nudged = Point(
          ...
          current = if (boundaryPoly != null && !insideBoundary(nudged, boundaryPoly)) {
            Point(
    ...
    for {
      ...
      settlement <- ZIO.attempt {
        ...
        val districts = dieRolls.zip(points).zipWithIndex.map { case ((roll, point), idx) =>
          ...
          val buildingTarget =
            math.max(6, math.min(50, (baseTarget * roadBias * (1.0 + hubBias) * edgeBias).round.toInt))
    ...
    def estimateHeight(fontSize: Int): Int = {
      ...
      settlement.districts.foldLeft(0) { (acc, district) =>
        ...
        val poiLines = district.pointsOfInterest.foldLeft(0) { (poiAcc, poi) =>
          val detail = poi.tavern
            .map(t => s"${t.name} - ${t.knownFor}")
...
object SettlementServer {
  ...
  val live: ZLayer[
    NameServer
      ...
      with NpcQualityServer
      with SettlementNameServer,
    Nothing,
    SettlementServer
  ] =
