...
object TitleRepositoryLive {

  private implicit val ec: ExecutionContext = ExecutionContext.global

  private val codecRegistry =
    fromRegistries(
      fromProviders(Macros.createCodecProviderIgnoreNone[Title]),
  ...
  private def resolveCollection(cfg: com.typesafe.config.Config): String =
    if (cfg.hasPath("mongo.title.collection")) cfg.getString("mongo.title.collection")
    else if (cfg.hasPath("mongodb.title.collection")) cfg.getString("mongodb.title.collection")
    else "Titles"

  val layer: ZLayer[Any, Throwable, TitleRepository] = ZLayer.scoped {
    for {
      cfg <- loadConfig
      uri <- resolveUri(cfg)
      ...
      coll <- ZIO.attempt {
        client
          ...
          .getCollection[Title](collection)
      }
    } yield new TitleRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(title: Title): Task[Title] =
        fromFuture(coll.insertOne(title).toFuture()).as(title)

      override def findById(id: ObjectId): Task[Option[Title]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[Title]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(title: Title): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", title._id), title, ReplaceOptions().upsert(false))
            .toFuture()
