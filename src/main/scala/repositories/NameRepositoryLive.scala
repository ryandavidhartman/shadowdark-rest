...
object NameRepositoryLive {

  private implicit val ec: ExecutionContext = ExecutionContext.global

  private val codecRegistry =
    fromRegistries(
      fromProviders(Macros.createCodecProviderIgnoreNone[Name]),
  ...
  val layer: ZLayer[Any, Throwable, NameRepository] = ZLayer.scoped {
    for {
      cfg <- loadConfig
      uri <- resolveUri(cfg)
    ...
    } yield new NameRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(name: Name): Task[Name] =
        fromFuture(coll.insertOne(name).toFuture()).as(name)

      override def findById(id: ObjectId): Task[Option[Name]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[Name]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(name: Name): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", name._id), name, ReplaceOptions().upsert(false))
            .toFuture()
