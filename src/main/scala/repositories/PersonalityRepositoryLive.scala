...
object PersonalityRepositoryLive {

  private implicit val ec: ExecutionContext = ExecutionContext.global

  private val codecRegistry =
    fromRegistries(
      fromProviders(Macros.createCodecProviderIgnoreNone[Personality]),
  ...
  private def resolveCollection(cfg: com.typesafe.config.Config): String =
    if (cfg.hasPath("mongo.personality.collection")) cfg.getString("mongo.personality.collection")
    else if (cfg.hasPath("mongodb.personality.collection")) cfg.getString("mongodb.personality.collection")
    else "Personalities"

  val layer: ZLayer[Any, Throwable, PersonalityRepository] = ZLayer.scoped {
    for {
      cfg <- loadConfig
      uri <- resolveUri(cfg)
      ...
      coll <- ZIO.attempt {
        client
          ...
          .getCollection[Personality](collection)
      }
    } yield new PersonalityRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(personality: Personality): Task[Personality] =
        fromFuture(coll.insertOne(personality).toFuture()).as(personality)

      override def findById(id: ObjectId): Task[Option[Personality]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[Personality]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(personality: Personality): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", personality._id), personality, ReplaceOptions().upsert(false))
            .toFuture()
