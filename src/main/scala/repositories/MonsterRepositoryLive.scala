...
import models.{Monster, MonsterSpecial, MonsterStats}
...
object MonsterRepositoryLive {
  ...
  private val codecRegistry =
    fromRegistries(
      fromProviders(
        Macros.createCodecProviderIgnoreNone[MonsterSpecial],
        Macros.createCodecProviderIgnoreNone[MonsterStats],
        Macros.createCodecProviderIgnoreNone[Monster],
      ),
  ...
  val layer: ZLayer[Any, Throwable, MonsterRepository] = ZLayer.scoped {
    for {
      cfg        <- loadConfig
      uri        <- resolveUri(cfg)
      ...
      coll       <- ZIO.attempt {
                      client
                        ...
                        .getCollection[Monster](collection)
                    }
    } yield new MonsterRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(monster: Monster): Task[Monster] =
        fromFuture(coll.insertOne(monster).toFuture()).as(monster)

      override def findById(id: ObjectId): Task[Option[Monster]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[Monster]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(monster: Monster): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", monster._id), monster, ReplaceOptions().upsert(false))
            .toFuture(),
