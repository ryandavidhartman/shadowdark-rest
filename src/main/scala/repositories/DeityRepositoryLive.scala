...
object DeityRepositoryLive {

  private implicit val ec: ExecutionContext = ExecutionContext.global

  private val codecRegistry =
    fromRegistries(
      fromProviders(Macros.createCodecProviderIgnoreNone[Deity]),
  ...
  private def resolveCollection(cfg: com.typesafe.config.Config): String =
    if (cfg.hasPath("mongo.deity.collection")) cfg.getString("mongo.deity.collection")
    else if (cfg.hasPath("mongodb.deity.collection")) cfg.getString("mongodb.deity.collection")
    else "Deities"

  val layer: ZLayer[Any, Throwable, DeityRepository] = ZLayer.scoped {
    for {
      cfg <- loadConfig
      uri <- resolveUri(cfg)
      ...
      coll <- ZIO.attempt {
        client
          ...
          .getCollection[Deity](collection)
      }
    } yield new DeityRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(deity: Deity): Task[Deity] =
        fromFuture(coll.insertOne(deity).toFuture()).as(deity)

      override def findById(id: ObjectId): Task[Option[Deity]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[Deity]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(deity: Deity): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", deity._id), deity, ReplaceOptions().upsert(false))
            .toFuture()
