...
import models.{Race, RaceAbility}
...
object RaceRepositoryLive {
  ...
  val layer: ZLayer[Any, Throwable, RaceRepository] = ZLayer.scoped {
    for {
      ...
      override def create(race: Race): Task[Race] =
        fromFuture(coll.insertOne(race).toFuture()).as(race)

      override def findById(id: ObjectId): Task[Option[Race]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[Race]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(race: Race): Task[Boolean] =
        fromFuture(
          coll
