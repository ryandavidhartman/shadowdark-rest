...
object LanguageRepositoryLive {

  private implicit val ec: ExecutionContext = ExecutionContext.global

  private val codecRegistry =
    fromRegistries(
      fromProviders(Macros.createCodecProviderIgnoreNone[LanguageEntry]),
  ...
  private def resolveCollection(cfg: com.typesafe.config.Config): String =
    if (cfg.hasPath("mongo.language.collection")) cfg.getString("mongo.language.collection")
    else if (cfg.hasPath("mongodb.language.collection")) cfg.getString("mongodb.language.collection")
    else "Languages"

  val layer: ZLayer[Any, Throwable, LanguageRepository] = ZLayer.scoped {
    for {
      cfg        <- loadConfig
      uri        <- resolveUri(cfg)
      ...
      coll       <- ZIO.attempt {
                      client
                        ...
                        .getCollection[LanguageEntry](collection)
                    }
    } yield new LanguageRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(language: LanguageEntry): Task[LanguageEntry] =
        fromFuture(coll.insertOne(language).toFuture()).as(language)

      override def findById(id: ObjectId): Task[Option[LanguageEntry]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[LanguageEntry]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(language: LanguageEntry): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", language._id), language, ReplaceOptions().upsert(false))
            .toFuture(),
