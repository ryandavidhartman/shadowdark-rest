...
object SettlementNameRepositoryLive {

  private implicit val ec: ExecutionContext = ExecutionContext.global

  private val codecRegistry =
    fromRegistries(
      fromProviders(Macros.createCodecProviderIgnoreNone[SettlementName]),
  ...
  private def resolveCollection(cfg: com.typesafe.config.Config): String =
    if (cfg.hasPath("mongo.settlementName.collection")) cfg.getString("mongo.settlementName.collection")
    else if (cfg.hasPath("mongodb.settlementName.collection")) cfg.getString("mongodb.settlementName.collection")
    else "SettlementNames"

  val layer: ZLayer[Any, Throwable, SettlementNameRepository] = ZLayer.scoped {
    for {
      cfg        <- loadConfig
      uri        <- resolveUri(cfg)
      ...
      coll       <- ZIO.attempt {
                      client
                        ...
                        .getCollection[SettlementName](collection)
                    }
    } yield new SettlementNameRepository {

      private def fromFuture[A](fa: => scala.concurrent.Future[A]): Task[A] =
        ZIO.fromFuture(_ => fa)

      override def create(settlementName: SettlementName): Task[SettlementName] =
        fromFuture(coll.insertOne(settlementName).toFuture()).as(settlementName)

      override def findById(id: ObjectId): Task[Option[SettlementName]] =
        fromFuture(coll.find(Filters.equal("_id", id)).headOption())

      override def list(): Task[List[SettlementName]] =
        fromFuture(coll.find().toFuture()).map(_.toList)

      override def update(settlementName: SettlementName): Task[Boolean] =
        fromFuture(
          coll
            .replaceOne(Filters.equal("_id", settlementName._id), settlementName, ReplaceOptions().upsert(false))
            .toFuture(),
