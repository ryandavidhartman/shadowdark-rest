...
object GearUtilities {
  ...
  def selectStartingGear(
    ...
    val crawlingKitItems =
      List("Backpack", "Flint and Steel", "Torch", "Rations", "Iron Spikes (10)", "Grappling Hook", "Rope, 60'")
    ...
    if (remainingSlots > 0) {
      val extraCrawling =
        random.shuffle(crawlingKitItems).map(canonicalName).filterNot(n => usedNames.contains(n)).take(remainingSlots)
